...
// Loading state
const isVoteLoading = computed(() => gameStore.isActionLoading('village_vote'))

// Can player vote?
const canVote = computed(() => isVotePhase.value && isAlive.value && voteState.value.active && !isVoteLoading.value)
...
<template>
    <div v-if="voteState.active" class="vote-panel space-y-4">
        ...
        <div class="vote-header flex items-center justify-between border-b border-red-900 pb-3">
            ...
            <span v-if="isVoteLoading" class="text-sm text-yellow-400 flex items-center gap-2">
            ...
            <span v-else-if="hasVoted" class="text-sm text-green-400 flex items-center gap-1">
