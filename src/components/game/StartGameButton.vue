...
// Loading state
const isLoading = computed(() => gameStore.isActionLoading('start_game'))

// Can the game be started?
const canStart = computed(() => {
    return isHost.value && isWaiting.value && rolesMatchPlayers.value && playerCount.value >= 4 && !isLoading.value
})
...
<template>
    <div v-if="isHost && isWaiting" class="start-game-section space-y-3">
        ...
        <button
            ...
            :class="{
                'bg-gradient-to-r from-green-700 to-green-600 hover:from-green-600 hover:to-green-500 text-white cursor-pointer shadow-lg hover:shadow-xl active:scale-[0.98]': canStart && !isLoading,
                'bg-gradient-to-r from-yellow-700 to-yellow-600 text-white cursor-wait': isLoading,
                'bg-gray-700 text-gray-500 cursor-not-allowed': !canStart && !isLoading
            }"
        >
            <span class="flex items-center justify-center gap-3">
                ...
                <!-- Play icon -->
                <svg v-else-if="canStart" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span>{{ isLoading ? 'Lancement...' : 'Lancer la Partie' }}</span>
            </span>
        </button>
